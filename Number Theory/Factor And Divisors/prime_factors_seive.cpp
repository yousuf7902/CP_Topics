...
int main(){
    ...
    // For prime factorization we can use highest prime factor or lowest prime factor

    //Prime factorization with prime numbers [ Time complexity O(log(n)) }
    ...
    while(num>1){
        ...
        while(num%p_num==0){
            num /= p_num;
            prime_factors.emplace_back(p_num);
        }
    }
